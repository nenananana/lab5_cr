...
{
    /// <summary>
    /// Представляет базу данных, содержащую художников, стили и картины,
    /// загружаемую и сохраняемую в Excel-файл.
    /// </summary>
    class Database
    {
        ...
        /// и загружает данные из указанного Excel-файла.
        /// </summary>
        /// <param name="filePath">Путь к Excel-файлу (.xls).</param>
        /// <exception cref="FileNotFoundException">Выбрасывается, 
        /// если файл не найден.</exception>
        /// <exception cref="ArgumentException">Выбрасывается, 
        /// если файл не имеет расширение .xls.</exception>
        public Database(string filePath)
        {
            ...
            if (!filePath.EndsWith(".xls",
                ...
                throw new ArgumentException(
                    "Тип файла должен быть .xls!");

            Workbook workbook = new(filePath);

            _artists = new List<Artist>();
            Worksheet artistsWorksheet =
                workbook.Worksheets["Художники"];
            for (int rowIndex = 1;
                 rowIndex <= artistsWorksheet.Cells.MaxDataRow;
                 rowIndex++)
            {
                Row currentRow =
                    artistsWorksheet.Cells.Rows[rowIndex];
                _artists.Add(new Artist(
                    currentRow[0].IntValue,
                    currentRow[1].StringValue));
            }

            _styles = new List<Style>();
            Worksheet stylesWorksheet =
                workbook.Worksheets["Стиль"];
            for (int rowIndex = 1;
                 rowIndex <= stylesWorksheet.Cells.MaxDataRow;
                 rowIndex++)
            {
                Row currentRow =
                    stylesWorksheet.Cells.Rows[rowIndex];
            ...
            Worksheet paintingsWorksheet =
                workbook.Worksheets["Картины"];
            for (int rowIndex = 1;
                 rowIndex <= paintingsWorksheet.Cells.MaxDataRow;
                 rowIndex++)
            {
                Row currentRow =
                    paintingsWorksheet.Cells.Rows[rowIndex];
                try
                {
                    ...
                    int year = currentRow[4].Type ==
                        CellValueType.IsNull ?
                        0 : currentRow[4].IntValue;
                    int styleId = currentRow[5].IntValue;

                    _paintings.Add(new Painting(
                        id, name, artistId,
                        hermitagePart, year, styleId));
                }
                catch (Exception ex)
                {
                    Console.WriteLine(
                        $"Ошибка при чтении строки " +
                        $"{rowIndex + 1}: {ex.Message}");
                }
            }
        }
        ...
        public void AddPainting(string name, int artistId, 
            int hermitagePart, int year, int styleId)
        {
            int nextId = _paintings.Count > 0 ? 
                _paintings[^1].Id + 1 : 1;
            _paintings.Add(new Painting(nextId, name, 
                artistId, hermitagePart, year, styleId));
            Save();
        }
        ...
        /// <exception cref="ArgumentNullException">Если <paramref name="logger"/> 
            ...
            Protocoler logger)
        {
            switch (entityType)
            {
                case EntityType.Artist:
                    var artist = _artists.FirstOrDefault(
                        x => x.Id == id);
                    if (artist != null)
                    {
                        _artists.Remove(artist);
                        logger.WriteLine(
                            $"Удалён художник: {artist}");
                        Save();
                        return true;
                    }
                    break;
                ...
                case EntityType.Painting:
                    var painting = _paintings.FirstOrDefault(
                        x => x.Id == id);
                    if (painting != null)
                    {
                        _paintings.Remove(painting);
                        logger.WriteLine(
                            $"Удалена картина: {painting}");
                        Save();
                        return true;
                    }
                    break;
            ...
            Protocoler logger)
        {
            ...
            {
                logger.WriteLine(
                    $"Изменено имя художника: {artist.Name} → {newName}");
                artist.Name = newName;
                Save();
                return true;
            }
        ...
        /// <param name="logger">Экземпляр логгера для 
        /// записи действия.</param>
        /// <returns>True, если редактирование 
        /// прошло успешно; иначе — false.</returns>
           ...
           Protocoler logger)
        {
            ...
            {
                logger.WriteLine(
                    $"Изменено имя стиля: {style.Name} → {newName}");
                style.Name = newName;
                Save();
                return true;
            }
        ...
        /// <param name="logger">Экземпляр логгера для записи действия.</param>
        /// <returns>True, если редактирование прошло успешно; иначе — false.</returns>
        public bool EditPainting(
            int id,
            string newName,
            int newArtistId,
            int newHermitagePart,
            int newYear,
            int newStyleId,
            Protocoler logger)
        {
        ...
        /// <summary>
        /// Возвращает количество художников, у которых 
        /// более 5 картин во 2-й части Эрмитажа.
        /// </summary>
        ...
        public int CountArtistsWithMoreThan5PaintingsInPart2()
        {
            var query = from painting in _paintings
                        ...
                        group painting by painting.IdArtist
                        into artistGroup
        ...
        /// <summary>
        /// Возвращает список картин, авторы которых 
        /// содержат указанную подстроку в имени.
        /// </summary>
        /// <param name="searchString">Подстрока для 
        /// поиска в имени автора.</param>
        /// <returns>Список объектов с названием картины и 
        /// именем автора.</returns>
        public IEnumerable<object> GetPaintingsWithAuthorNameContaining(
            string searchString)
        {
            return from painting in _paintings
                   join artist in _artists
                   on painting.IdArtist equals artist.Id
                   where artist.Name.Contains(
                       searchString,
                       StringComparison.OrdinalIgnoreCase)
                   select new
                   {
        ...
        /// <returns>Список объектов с названием картины, 
        /// именем автора и годом создания.</returns>
        public IEnumerable<object> GetPaintingsByStyleName(
            string styleName)
        {
            return from painting in _paintings
                   join artist in _artists
                   on painting.IdArtist equals artist.Id
                   join style in _styles
                   on painting.IdStyle equals style.Id
                   where style.Name.Equals(
                       styleName,
                       StringComparison.OrdinalIgnoreCase)
                   select new
                   {
        ...
        public void Save()
        {
            ...
            Worksheet artistsWorksheet =
                workbook.Worksheets["Художники"];
            artistsWorksheet.Cells.DeleteRows(
                1,
                artistsWorksheet.Cells.Rows.Count - 1);
            int rowIndex = 1;
            foreach (var artist in _artists)
            {
                artistsWorksheet.Cells[rowIndex, 0]
                    .PutValue(artist.Id);
                artistsWorksheet.Cells[rowIndex, 1]
                    .PutValue(artist.Name);
                rowIndex++;
            }

            Worksheet stylesWorksheet =
                workbook.Worksheets["Стиль"];
            stylesWorksheet.Cells.DeleteRows(
                1,
                stylesWorksheet.Cells.Rows.Count - 1);
            rowIndex = 1;
            foreach (var style in _styles)
            {
                stylesWorksheet.Cells[rowIndex, 0]
                    .PutValue(style.Id);
                stylesWorksheet.Cells[rowIndex, 1]
                    .PutValue(style.Name);
                rowIndex++;
            }
            ...
            Worksheet paintingsWorksheet =
                workbook.Worksheets["Картины"];
            paintingsWorksheet.Cells.DeleteRows(
                1,
                paintingsWorksheet.Cells.Rows.Count - 1);
            rowIndex = 1;
            foreach (var painting in _paintings)
            {
                paintingsWorksheet.Cells[rowIndex, 0]
                    .PutValue(painting.Id);
                paintingsWorksheet.Cells[rowIndex, 1]
                    .PutValue(painting.Name);
                paintingsWorksheet.Cells[rowIndex, 2]
                    .PutValue(painting.IdArtist);
                paintingsWorksheet.Cells[rowIndex, 3]
                    .PutValue(painting.ErmitagePart);
                paintingsWorksheet.Cells[rowIndex, 4]
                    .PutValue(painting.Year);
                paintingsWorksheet.Cells[rowIndex, 5]
                    .PutValue(painting.IdStyle);
                rowIndex++;
            }
